...
def default():
  ...
  for n in range (0, 9):
    ...
    if len(result[n][4]) > length:
       ...
       print "<b><a href=\"index.py?action=show&queue=" + str(queue) + "&msgid=" + str(result[n][5]) + "\">View Full record</a> ] </b><br>\n"
    else:
...
def day():
  ...
  if len(result) != 0:
     ...
     for msgid in result:
        ...
        print "<a href=\"index.py?action=show&queue=" + str(queue) + "&msgid=" + str(recordinfo[0][5]) + "\">View Full record</a><br>\n"
     footer()
  # if not, sorry!
  else:
...
def searchsub():
  ...
  if len(recordids) != 0:
     ...
     for recordinfo in results:
       ...
       print "<a href=\"index.py?action=show&queue=" + str(queue) + "&msgid=" + str(recordinfo[5]) + "\">View Full record</a><br>\n"
  footer()
